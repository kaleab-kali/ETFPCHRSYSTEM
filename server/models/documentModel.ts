...
export interface LetterDocument extends Document {
  ...
  center?: string;
  date?: Date;
  direction?: "in" | "out"; // Restricting the values to "in" or "out"
  file?: string;
}
...
const letterSchema = new Schema<LetterDocument>(
  {
    ...
    direction: {
      type: String,
      enum: ["in", "out"], // Validating the direction field to only accept "in" or "out"
      required: true, // Making sure this field is always provided
    },
    file: { type: String },
  },
  { timestamps: true }
