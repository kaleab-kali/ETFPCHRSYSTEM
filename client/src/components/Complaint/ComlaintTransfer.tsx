...
import { useUpdateTransferComplaint } from "../../services/mutations/complaintMutation";
...
const ComplaintTransfer: React.FC = () => {
  const allComplaints = useAllComplaints();
  const transferOrReject = useUpdateTransferComplaint()
  ...
  const handleAccept = (complaintId: string) => {
    ...
    transferOrReject.mutate(complaintData);
  ...
  const pendingComplaints = allComplaints.data.filter(
    (complaint: Complaint) => complaint.status?.toUpperCase() === "SENT"
  );
